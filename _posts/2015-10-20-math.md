...
We use \(\mathcal{R}_p\) to denote the set of all rational proper transfer functions in \(z\)-domain and \(\mathcal{RH}_\infty\) to denote the set of all rational proper stable transfer functions. \(\mathcal{RH}_\infty = \mathcal{RH}_2\) in discrete-time context, since they are all equivalent to have all their poles in \(\mathbb{D}\).
\begin{defn}
Given an LTI MIMO (\(m\times n\)) operator \(\hat{G}\), its \(\ell_1\) norm is defined as:
\begin{equation}
    \|\hat{G}\|_1 \triangleq \|G\|_1 =  \underset{1\leq i\leq m}{\max} \sum_{j=1}^n \sum_{t=0}^\infty |G_{ij}[t]|
\end{equation}
where \(G = \mathcal{Z}^{-1}(\hat{G})\) is its impulse response.
Furthermore, \(\ell_1^{m\times n}\) defines the set of all \(m\times n\) matrix sequences \(G\) such that \(\|G\|_1 < \infty\).
\end{defn}
